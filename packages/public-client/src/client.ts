...
export class PublicClient implements PublicClientControl {
  ...
  constructor(ports: PublicClientPorts, options: PublicClientOptions = {}) {
    ...
    if (
      ...
      throw publicClientError(
        "PC_CONFIGURATION",
        "Clock skew is outside the supported bounds",
        {
          operation: "configure",
        },
      );
  ...
  async bootstrap(input: BootstrapInput): Promise<BootstrapResponse> {
    const prepared = protocolBoundary("bootstrap", () =>
      parseBootstrapLinkBinding(input.prepared),
    );
    if (
    ...
    const parsed = protocolBoundary("bootstrap", () =>
      parseBootstrapResponse(response.body),
    );
    if (
  ...
  async submitContribution(
    ...
    if (payload.installationId !== installation.bootstrap.installationId) {
      throw publicClientError(
        "PC_SCOPE_MISMATCH",
        "Contribution installation does not match",
        {
    ...
    if (receipt.contributionType !== intent.contributionType) {
      throw publicClientError(
        "PC_SCOPE_MISMATCH",
        "Receipt contribution type does not match",
        {
          operation: "submit-contribution",
        },
      );
  ...
  async getContributionStatus(
    ...
    if (receipt.contributionId !== contributionId) {
      throw publicClientError(
        "PC_SCOPE_MISMATCH",
        "Receipt contribution does not match",
        {
          operation: "contribution-status",
        },
      );
  ...
  async getLocalizationDemandStatus(
    contributionId: string,
    signal?: AbortSignal,
    ...
    const response = await this.controlWithRetry({
      ...
      path: CONTROL_PATHS.localizationDemandStatus(contributionId),
      method: "GET",
      body: null,
      credential: installation.bootstrap.intakeCredential,
      retryableOperation: true,
    ...
    const demand = protocolBoundary("localization-demand-status", () =>
      parseLocalizationDemandStatus(response.body),
    );
    if (demand.contributionId !== contributionId) {
      throw publicClientError(
        "PC_SCOPE_MISMATCH",
        "Demand status contribution does not match",
        {
          operation: "localization-demand-status",
        },
      );
  ...
  private assertReceiptEpoch(
    ...
    if (
      ...
      throw publicClientError(
        "PC_SCOPE_MISMATCH",
        "Receipt credential epoch does not match",
        {
          operation,
        },
      );
    ...
    try {
      ...
      if (bootstrap.installationState !== "active") {
        throw publicClientError(
          ...
          "Public installation is not active",
          {
            operation,
          },
        );
      ...
      if (error instanceof Error && error.name === "PublicClientError")
        throw error;
      throw publicClientError(
        "PC_STORAGE",
  ...
  private assertCredential(
    ...
    if (
      ...
      throw publicClientError(
        "PC_CLOCK_SKEW",
        "Public credential is issued in the future",
        {
          operation,
        },
      );
    ...
    if (
      credential.capabilities.some(
        (item) => !bootstrap.availableCapabilities.includes(item),
      )
    ) {
      throw publicClientError(
        "PC_SCOPE_MISMATCH",
        "Public credential scope does not match",
        {
          operation,
        },
      );
    ...
    if (
      ...
      throw publicClientError(
        "PC_CREDENTIAL_AUDIENCE",
        "Public credential lacks capability",
        {
          operation,
        },
      );
