...
- 选择一个、多个或全部已启用的社区插件。
- 选择其他插件的译文语言；语枢自身界面支持简体中文和英语，其他选择回退英语。
- 将已发布译文应用于插件名称、说明、设置、命令和界面文本；README 文档翻译不属于当前范围。
- 关闭本地化会立即恢复运行时文本；若使用过兼容补丁，文件恢复后需重新加载目标插件，外部改动产生的冲突会保留并提示。
- 默认保持插件文件和 Vault 笔记不变；高级兼容补丁仅按上述显式范围写入第三方插件静态文件。
...
Trans-Hub Localizer applies verified translations to supported community-plugin names, descriptions, settings, commands, and interface text. By default it changes only Obsidian's presentation layer. Vault notes remain untouched; third-party files are written only when you explicitly apply a compatibility patch.
...
Translations are matched and applied at runtime to Obsidian's presentation layer by default. It also excludes Markdown editors, reading views, code, scripts, and editable content; turning localization off restores the original presentation immediately.

The advanced compatibility patch is desktop-only, off by default, and needs an explicit click for each plugin. It writes only static text with an exact plugin version, published translation, and location proof, and saves a verifiable backup first; version changes or external modifications remain untouched and are reported as conflicts. It never modifies vault notes.

## What it does

- Select one, multiple, or all enabled community plugins.
- Choose a translation language for other plugins. The Trans-Hub interface supports Simplified Chinese and English and falls back to English for other selections.
- Apply published translations to plugin names, descriptions, settings, commands, and interface text. README translation is outside the current scope.
- Disable localization to restore runtime text immediately. After a compatibility patch is restored, reload the affected plugin; conflicts with external changes are preserved and reported.
- Keep plugin files and vault notes unchanged by default; the advanced compatibility patch has the explicit, narrow exception described above.
